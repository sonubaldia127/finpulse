...
async function syncRealData() {
    ...
    // Wipe old historical prices and stocks to avoid unique key conflicts
    await supabase.from('historical_prices').delete().neq('ticker', 'TEMP_SEED');
    await supabase.from('stocks').delete().neq('ticker', 'TEMP_SEED');

    console.log("\nFetching real-time and 1-Year historical data from Yahoo Finance...");
    ...
    for (const ticker of tickers) {
        try {
            ...
            // Upload the clean price rows in a single batch insert
            if (priceRows.length > 0) {
                const { error: histErr } = await supabase.from('historical_prices').insert(priceRows);
                if (histErr) throw histErr;
            }

            console.log(`✓ Factual data synced for ${ticker}`);

        } catch (error) {
            // Unlisted, private, or suspended tickers fail gracefully here without stopping the sync
            console.error(`✕ Failed to sync ${ticker}: ${error.message}`);
        }

        // Add a 500ms delay to respect Yahoo Finance API rate limits
        await new Promise(resolve => setTimeout(resolve, 500));
    }

    console.log("\nDatabase populated perfectly with real, untampered data!");
    process.exit();
}
